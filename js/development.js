document.querySelectorAll('button.load-stock').forEach(button => {
  ...
  const countTemplate = document.querySelector('.stock-content-template'); // Define the content template
  ...
  try {
    ...
    // Check if data from fetch is empty
    if (!data || data.length === 0) {
    window.alert('There is stock data but it\'s empty.');
    return; // Exit if no data is available
    }

    // Clone the content template for displaying stock items
    const contentClone = document.importNode(countTemplate.content, true); // Clone the content template content

    // Populate number of stock items heading
    contentClone.querySelector('.number-of-stock').textContent = `(${data.length}) Stock Items`;

    // Append the cloned content template to stockItemsListWrapper
